...
export const fetchPokemons = async (
  ...
  const response = await axios.get<IPokemonList>(`${process.env.VITE_POKEDEX_API}/pokemon`, {
    params: {
      page,
...
export const fetchPokemon = async (id: string): Promise<IPokemon> => {
  const response = await axios.get<IPokemon>(`${process.env.VITE_POKEDEX_API}/pokemon/${id}`);
...
export const fetchPokemonDetails = async (id: string): Promise<IPokemonDetails> => {
  const response = await axios.get<IPokemonDetails>(`${process.env.VITE_POKEDEX_API}/details/${id}`);
